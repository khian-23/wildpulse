...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _fetchDashboardOverview() async {
    ...
    try {
      final response = await AppApi.getAdmin(
        '/dashboard/overview',
        queryParameters: AppApi.reportQuery({'days': '7'}),
      );
      if (!mounted) return;
        ...
        setState(() {
          ...
          _dashboardError =
              response.statusCode == 401
                  ...
                  : 'Dashboard load failed: ${response.statusCode}';
        });
        return;
      }
  ...
  Future<void> _fetchConservationList({String? query}) async {
    ...
    try {
      ...
      final response = await AppApi.getAdmin(
        '/conservation/list',
        queryParameters: AppApi.reportQuery({'limit': '50', 'q': q}),
      );
      if (!mounted) return;
  ...
  Widget _buildDailyTrendChart(List<dynamic> trend) {
    final counts =
        trend.map((e) => (e['count'] as num?)?.toDouble() ?? 0).toList();
    final maxCount =
        counts.isEmpty ? 1.0 : counts.reduce((a, b) => a > b ? a : b);
            ...
            child: Row(
              ...
              children:
                  trend.map((item) {
                    ...
                    final day =
                        (item['day']?.toString() ?? '--').split('-').last;
                    final barHeight =
                        maxCount == 0 ? 2.0 : (count / maxCount) * 60;
                    return Expanded(
                      child: Padding(
    ...
    return ListView(
      ...
      children: [
                ...
                children: [
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      _metricTile('Alerts', totals['alerts'], Colors.redAccent),
                    ],
                  ),
                  const SizedBox(height: 10),
        ...
        ..._conservationList.map((animal) {
          final commonName =
              animal['commonName']?.toString() ?? 'Unknown Animal';
